...
document.querySelector('.version').textContent='v0.1.035 · Agenda réel';
document.querySelector('.demo-panel summary').textContent='Démo v0.1.035';
...
function displayAgendaTitle(e){
 // Règle v0.1.035 : conserver le titre Agenda tel quel.
 // Si la description contient une année (ex. "Anniversaire 1948" / "Mariage 1974"),
 // ajouter uniquement le nombre d’années, sans réécrire le titre.
 const title=(e.title||'').trim();
 const desc=(e.description||'').trim();
 const years=[...desc.matchAll(/\b(18\d{2}|19\d{2}|20\d{2})\b/g)].map(m=>+m[1]);
 if(!years.length) return title;
 const eventYear=e.start instanceof Date&&!isNaN(e.start)?e.start.getFullYear():new Date().getFullYear();
 const originYear=years[0];
 const age=eventYear-originYear;
 if(age<0||age>200) return title;
 // Ne pas doubler un nombre déjà présent à la fin du titre.
 if(new RegExp('(?:^|\\s)'+age+'\\s*$').test(title)) return title;
 return `${title} ${age}`;
}
