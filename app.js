...
document.querySelector('.version').textContent='v0.1.020 · Agenda intelligent';
document.querySelector('.demo-panel summary').textContent='Démo v0.1.020';
...
// v0.1.020 — agenda Android compact, sans tâches
...
function birthdayParts(e){
 let t=e.title.trim(),age='';
 if(kind(e)==='🎂'){
   let m=t.match(/\s+(\d{1,3})\s*$/); if(m){age=m[1];t=t.slice(0,m.index).trim()}
   t=t.replace(/\s+[-–—]?\s*(anniversaire|birthday|geburtstag)\s*$/i,'').trim();
   t=t.replace(/^(Mlle|Mme|M\.|Monsieur|Madame)\s+/i,'').trim();
 }
 return {title:t,age};
}
function cleanTitle(e){return birthdayParts(e).title}
function allDayHtml(e){
 const b=birthdayParts(e), age=b.age?` <span class="birthday-age">${esc(b.age)}</span>`:'';
 return `<div class="appointment all-day compact-row"><time>${kind(e)}</time><div><strong>${esc(b.title)}${age}</strong>${e.place?`<small>${esc(e.place)}</small>`:''}</div></div>`
}
...
function tomorrowHtml(e){
 ...
 let extra=[`jusqu’à ${hm(e.end)}`,e.place].filter(Boolean).join(' · ');
...
function installV020Layout(){
 ...
 st.textContent=`
   ...
   .tomorrow-card .timed-tomorrow strong{display:block;white-space:normal;overflow:visible}
