...
document.querySelector('.version').textContent='v0.1.034 · Agenda réel';
document.querySelector('.demo-panel summary').textContent='Démo v0.1.034';
...
function allDayHtml(e){
 // Règle v0.1.034 : l’Agenda Android est la source. Le titre est affiché tel quel, sans réécriture ni calcul.
 return `<div class="appointment all-day compact-row"><time>${kind(e)}</time><div><strong>${esc(e.title)}</strong>${e.place?`<small>${esc(e.place)}</small>`:''}</div></div>`
}
