/* Ma Vie v0.3.6 — Proposition : lieu, région, temps, activité */
(()=>{
 ...
 function ensureDialog(){
  ...
  d.innerHTML=`<form method="dialog" class="proposal-sheet"><div class="proposal-head"><strong>UNE PROPOSITION</strong><button value="cancel" class="proposal-close" aria-label="Fermer">×</button></div>
   ...
   <div class="proposal-pair"><label>Temps<select id="proposalTime"><option value="30">30 min</option><option value="60">1 h</option><option value="90">1 h 30</option><option value="120">2 h</option><option value="180">3 h</option></select></label><label>Activité<select id="proposalActivity"><option>Découverte / balade / terrasse</option><option>Café / terrasse</option><option>Restaurant / terrasse</option><option>Restaurant / intérieur</option><option>Marché / producteurs</option><option>Musée / exposition / intérieur</option><option>Nature / balade</option><option>Surprends-moi</option></select></label></div>
 ...
 function openProposal(day){
  ...
  const a=document.querySelector('#proposalActivity'),wanted=suggestedActivity(day);[...a.options].some(o=>{if(o.textContent===wanted){a.value=wanted;return true}return false});
 ...
 function search(day){
  ...
  const date=new Intl.DateTimeFormat('fr-CH',{weekday:'long',day:'numeric',month:'long'}).format(day);const where=region==='Indifférent'?place:`${region} ${place==='Près de moi'?'':place}`;
  const q=[act,where,date,`${mins} minutes`,wx.label].filter(Boolean).join(' ');document.querySelector('#proposalDialog')?.close();openUrl('https://www.google.com/search?q='+encodeURIComponent(q));
 }
 ...
 .mv-know{display:grid;grid-template-columns:136px minmax(0,1fr);column-gap:0;align-items:baseline;padding:5px 0}
 .mv-know+.mv-know{border-top:1px solid rgba(255,255,255,.52)}
 .mv-know-when{border-right:1px solid rgba(7,60,74,.58);padding-right:14px;font-size:.70rem;line-height:1.12;font-weight:800}
 .mv-know-when span{display:block}.mv-know-body{padding-left:18px;align-self:baseline}
 ...
 .agenda-empty-message{margin-left:82px!important;padding-left:0!important;font:inherit!important;font-size:.78rem!important;font-weight:700!important;line-height:1.2!important}
